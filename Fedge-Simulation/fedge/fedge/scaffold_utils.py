        ...
        with torch.no_grad():
            for name, local_param in local_model.named_parameters():
                if name in self.client_control:
                    ...
                    # Safety: Check for NaN and skip if detected
                    if torch.isnan(new_control).any():
                        logger.warning(f"NaN detected in control variate for {name}, skipping update")
                        continue

                    # Clip to prevent explosion with imbalanced data
        ...
        with torch.no_grad():
            for name, param in model.named_parameters():
                if param.grad is not None and name in self.client_control:
                    ...
                    # Safety: Skip if correction contains NaN/Inf
                    if torch.isnan(correction).any() or torch.isinf(correction).any():
                        logger.warning(f"NaN/Inf in SCAFFOLD correction for {name}, skipping")
                        continue
