#!/usr/bin/env python3
"""
Generate a 2x2 overview plot from federated FedProx metrics CSVs.
"""
    ...
    if clients_path.exists():
        try:
            ...
            for client_id, group in df.groupby('client_id'):
                per_client[str(client_id)] = group.copy()
        except Exception as e:
...
def make_overview_plot(
    ...
    """Create a 2x2 overview plot of FedProx metrics."""
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Generate FedProx overview plots from metrics CSVs.")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if not title:
        ...
        title = f"FedProx Overview — {exp_name}"
